...
const registrationSchema = z.object({
  ...
  parent1Phone: z.string().min(1, 'Parent/Guardian 1 phone is required'),
  parent1Email: z.string().default(''),
  parent2Name: z.string().default(''),
  parent2Phone: z.string().default(''),
  parent2Email: z.string().default(''),
...
export async function POST(request: NextRequest) {
  ...
  const { error: studentError } = await adminClient.from('students').insert({
    ...
    parent1_phone: data.parent1Phone,
    parent1_email: data.parent1Email,
    parent2_name: data.parent2Name,
    parent2_phone: data.parent2Phone,
